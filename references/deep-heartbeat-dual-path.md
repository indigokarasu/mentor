...
1. **Single-path scan** — only walks `<hermes-root>/commons/journals/`, missing ~50% of journals under `<hermes-home>/commons/journals/`
2. **`os.walk` unreliable in cron** — silently returns 0 results in cron `terminal()` heredocs (gotcha #17)

## Dual-Path Dedup Pattern
```python
COMMONS_JOURNALS = "<hermes-root>/commons/journals/"
PROFILE_JOURNALS = "<hermes-home>/commons/journals/"

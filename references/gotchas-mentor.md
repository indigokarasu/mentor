...
=======
24. **Profile-scoped journal paths resolve to `..` skill name** — `cron-heartbeat-light.py` computes skill name via `os.path.relpath(fpath, JOURNALS_DIR)` where `JOURNALS_DIR = "~/.hermes/commons/journals"`. Files under `~/.hermes/profiles/indigo/commons/journals/` resolve to `..` as the skill name. Fix: extract skill name from path parts after `/journals/`: `parts = fpath.split('/'); skill = parts[parts.index('journals')+1] if 'journals' in parts else 'unknown'`.
>>>>>>> Stashed changes
...
25b. **`active_skills_30d` in the script output reflects only the piped 3-day file set** — The script receives only `-mtime -3` files via stdin, so its raw `active_skills_30d` count (e.g., 12–13) is NOT the true 30-day active skill count (e.g., 18–22). **UPDATE 2026-06-21**: Despite earlier documentation (v2.8.23 release notes) claiming the script self-corrects, empirical testing across 11+ consecutive heartbeats (2026-06-19 through 2026-06-21) confirms the script does NOT self-correct. The evidence record never shows `correction: true`. The caller MUST always compute the dual-path 30-day count and write a corrected evidence record. Two evidence lines per heartbeat is the expected pattern. See gotcha #58.
...
77. **`grep` on evidence.jsonl reports "binary file matches" — use `-a` flag** — After evidence.jsonl grows beyond ~5,000 lines, `grep` may classify it as binary (due to long lines without newline characters, or non-ASCII UTF-8 in journal content). This produces the unhelpful output `grep: /path/to/evidence.jsonl: binary file matches` instead of matching lines. **Fix:** Always use `grep -a` (treat as text) when searching evidence.jsonl, ingestion_log.jsonl, or any large JSONL file: `grep -a "pattern" /path/to/evidence.jsonl`. Confirmed 2026-06-27: `grep "2026-06-27" evidence.jsonl` returned binary warning; `grep -a` returned results immediately.
...
39a. **Bulk scan-and-repair for evidence.jsonl — brace-depth + null-byte handling** — Gotcha #40 covers tail-line repair (removing only the last corrupt line). For systematic corruption throughout the file (concatenated JSON objects, null-byte lines, multi-line JSON), use the full scan-and-repair procedure in `references/evidence-log-maintenance.md`. Key technique: brace-depth tracking to find the boundary between two concatenated JSON objects on one line. Always repair BOTH profile and commons evidence independently (dual-repair requirement, confirmed 2026-07-01).
...
=======
    if ! tail -1 ~/.hermes/commons/data/mentor/evidence.jsonl | python3 -c "import sys,json; json.loads(sys.stdin.read())" 2>/dev/null; then
        # Remove corrupt tail — scan for last valid JSONL line (may be multiple corrupt lines)
        VALID_LINES=$(python3 -c "
import json
lines = open('~/.hermes/commons/data/mentor/evidence.jsonl').readlines()
>>>>>>> Stashed changes
...
=======
        head -n "$VALID_LINES" ~/.hermes/commons/data/mentor/evidence.jsonl > /tmp/evidence_repair.jsonl
        cp /tmp/evidence_repair.jsonl ~/.hermes/commons/data/mentor/evidence.jsonl
>>>>>>> Stashed changes
...
=======
    ")" >> ~/.hermes/commons/data/mentor/evidence.jsonl
>>>>>>> Stashed changes
...
=======
        cat /tmp/evidence_backup.json >> ~/.hermes/commons/data/mentor/evidence.jsonl
>>>>>>> Stashed changes
    else
...
=======
42. **Ingestion log uses TWO path formats — naive `comm` shows false "new" files** — The ingestion log stores paths in two formats: (a) relative `ocas-xxx/YYYY-MM-DD/file.json` (from early heartbeats) and (b) absolute `~/.hermes/commons/journals/ocas-xxx/...` (from later heartbeats). Profile-scoped paths use `~/.hermes/profiles/indigo/commons/journals/ocas-xxx/...`. A naive `comm -23 <(sort find_output) <(sort ingestion_log)` will show 800+ "new" files when only ~6 are truly new, because the relative paths from the ingestion log don't match the absolute paths from `find`. **Fix:** Extract and normalize all paths to absolute before comparison:
>>>>>>> Stashed changes
    ```bash
    python3 -c "
    import json
...
=======
    with open('~/.hermes/commons/data/mentor/ingestion_log.jsonl') as f:
>>>>>>> Stashed changes
        for line in f:
            try:
                d = json.loads(line)
...
=======
                        src = '~/.hermes/commons/journals/' + src
>>>>>>> Stashed changes
                    paths.add(src)
            except: pass
...
=======
    with open('~/.hermes/commons/data/mentor/ingestion_log.jsonl', 'w') as f:
>>>>>>> Stashed changes
...
44a. **`awk -F/` on absolute paths produces `//` double-slash prefixes — use `grep -oP` instead** — When counting unique skill directories from absolute paths using `awk -F/`, the leading `/` creates an empty first field, producing paths like `/~/.hermes/...` instead of `~/.hermes/...`. This causes downstream path comparisons to fail because `/<fs-root>/...` ≠ `<fs-root>/...`. Additionally, the awk approach requires complex logic to strip date subdirectories and deduplicate. **Confirmed reliable alternative:** Use `grep -oP` to extract the skill name directly from the path:
>>>>>>> Stashed changes
    ```bash
...
=======
53. **Shell `${VAR}` inside heredocs can produce stray braces in paths** — When constructing file paths in heredocs using `${VAR}` syntax, a stray `}` can appear (e.g., `${DATA_DIR}}` producing `/path/to/file.jsonl}`). This silently breaks file operations. **Use `$VAR` without braces for path interpolation in heredocs**, or better yet, use Python heredocs (`python3 << 'PYEOF'`) with hardcoded paths for all backup writes. Never mix shell variable expansion with `>>` redirection to paths constructed from variables. Confirmed 2026-06-14: three separate terminal() calls failed with "No such file or directory" because `${DATA_DIR}}` expanded to `~/.hermes/commons/data/mentor/}`.
>>>>>>> Stashed changes
...
55b. **`$VAR` inside `'PYEOF'` heredoc writes literal string to JSON — corruption requires tail-1 removal** — Distinct from gotcha #54 (env var returns empty string): here the Python `open(..., 'a').write()` *succeeds* but the *content* contains the literal `$VAR` string because the shell never expanded it. This produces valid JSON with semantically wrong values (e.g., `"run_id": "$RUN_ID"`). **Detection:** `tail -1 <file.jsonl` and check for shell variable syntax (`$VAR`, `${VAR}`) in string values. **Fix:** Remove the corrupted tail line and re-write with the value hardcoded. Confirmed 2026-06-21: correction evidence record written with literal `$RUN_ID`.
...
=======
66. **Commons evidence.jsonl can grow AHEAD of profile evidence.jsonl from prior direct-write patterns** — Gotcha #59 describes the expected pattern (profile leads commons). The reverse can also occur: commons evidence grows ahead of profile when prior sessions or tools write directly to `~/.hermes/commons/data/mentor/evidence.jsonl` without going through the profile path. Confirmed 2026-06-17: commons evidence was 2,586 lines while profile evidence was 2,562 lines — commons ahead by 24 lines. This is NOT an error — the line-level set-difference sync handles this correctly by only appending profile-lines-not-in-commons, so extra commons lines are preserved (not overwritten). The sync is one-directional: profile → commons append-only, never truncating commons. Do NOT attempt to "fix" this by copying commons back to profile or by truncating commons. The two files are allowed to diverge; commons is a superset that absorbs profile lines over time.

67. **Commons ingestion_log.jsonl accumulates massive historical bloat — 2-4x+ profile line count is normal** — The commons ingestion log receives writes from multiple sources: the heartbeat script (when it targets commons directly), caller backups (when incorrectly written to commons per gotcha #62), and the line-level sync from profile. Over time, commons ingestion_log grows to 2-4x the profile count. Confirmed 2026-06-18: commons ingestion was 28,393 lines vs profile 11,692 lines (2.4x). Confirmed 2026-06-19: commons ingestion was 41,409 lines vs profile 12,278 lines (3.4x) — the ratio continues to grow over time as commons accumulates historical entries from pre-profile-era direct writes. This is a known cosmetic issue — the profile ingestion_log is the authoritative source and is the one read by the heartbeat's cross-reference dedup. The commons copy is a lagging superset. Do NOT attempt to deduplicate or truncate commons ingestion_log — it has no operational impact (the heartbeat reads from profile). The gap will continue to grow slowly over time and may exceed 4x over months.

62. **Caller backup writes MUST target profile path, NOT commons — writing to commons creates duplicate/offset evidence lines** — When the caller writes backup evidence or ingestion records, the target MUST be the profile-scoped path (`~/.hermes/profiles/indigo/commons/data/mentor/`), NOT `~/.hermes/commons/data/mentor/`. The profile is the authoritative source; commons receives data only via the line-level set-difference sync run after each heartbeat. If the caller writes directly to commons: (a) the sync copies the script's version from profile (with wrong `active_skills_30d`) to commons, (b) the caller's corrected version is also in commons, (c) two evidence lines exist for the same run — one wrong, one correct. This confuses gap detection and metrics. **The rule: write once, to profile. Let sync handle commons.** Confirmed 2026-06-16: backup evidence written to commons path created duplicate lines; required re-writing to profile + re-sync.
>>>>>>> Stashed changes
...
## Heredoc Syntax — Terminal Background Detection

70. **`<<` heredoc syntax triggers terminal background detection — exit_code=-1** — The `terminal()` tool's foreground mode treats `<<` heredoc delimiters as background operators, even in simple single-command invocations like `cat > /tmp/file << 'EOF'`. This is distinct from gotcha #64 (ampersand in content) and gotcha #57 (`&&` between commands). The `<<` itself is what triggers it. **Fix:** Write files via `write_file` tool first, then invoke with `python3 /tmp/script.py`. For shell-level file writes, use `cat > /tmp/file` followed by separate `echo >> /tmp/file` lines, or use `printf` with `>>` redirection. Confirmed 2026-06-22: 4 consecutive `terminal()` calls with `<< 'PYEOF'` heredocs all failed with exit_code=-1; same commands written via `write_file` + `python3 /tmp/script.py` succeeded immediately.

71. **Ingestion log cross-reference: `set()` difference on raw lines undercounts — ingestion log uses path-normalized dedup** — When cross-referencing `find` output against `ingestion_log.jsonl` using Python `set()` difference on raw file paths, the ingestion log may show only a tiny fraction of 3-day files as "ingested" (e.g., 2 of 4,750). This is expected — the ingestion log stores paths in a normalized/deduped format, and the script's internal dedup logic (path normalization, field name matching) means many files are recognized as already-ingested even though their raw paths don't appear in the log. The script's `new_files_ingested` count is the reliable metric for this heartbeat's work. The cross-reference is still valuable for detecting large discrepancies but should not be used to override the script's count when the delta is small (≤10). Confirmed 2026-06-22: ingestion log had 2 of 4,750 3-day files; script correctly ingested 4 new files.

72. **Never chain `rm` with `;` followed by a read of the same file — the read fails silently** — When verifying a file after deletion, `rm /path/to/file 2>/dev/null; echo "ok" && wc -l < /path/to/file` will delete the file and then the `wc -l` reads a now-missing file, producing a confusing error or empty output. The `;` separator means both commands run regardless of `rm`'s exit code. **Fix:** Use `&&` between `rm` and subsequent commands (so the read only runs if `rm` succeeded — which you can then catch), or better yet, never `rm` and read in the same pipeline. For evidence.jsonl specifically: the file is critical — never `rm` it. If corruption is suspected, repair in-place (truncate to last valid JSONL line) rather than deleting. Confirmed 2026-06-22: `rm evidence.jsonl 2>/dev/null; wc -l < evidence.jsonl` deleted the file and the read returned "No such file or directory" — required restoring from commons backup.

73. **Caller must NOT write a separate journal even with a different run_id — the script's journal is canonical** — The `cron-heartbeat-light.py` script writes its own journal entry. The caller must NOT write an additional journal entry, even with a different timestamp/run_id. Writing a caller journal produces duplicate journals for the same heartbeat cycle, inflating `new_files_ingested` on the next scan (self-referential ingestion, gotcha #35). If the caller needs to record additional context, append it to the evidence record (gotcha #29 correction pattern), not as a separate journal file. Confirmed 2026-06-22: caller wrote `mentor-light-20260622T070912Z.json` in addition to the script's `mentor-light-20260622T070737Z.json`; required manual `rm` of the caller journal.
  - **Overwrite-then-destroy variant (confirmed 2026-06-24):** The agent wrote a duplicate journal via shell heredoc at the SAME path as the script's canonical journal (`mentor-light-20260624T183520Z.json`). The heredoc overwrote the script's already-written file. The checkpoint fired post-write; deletion of the duplicate also deleted the canonical journal. **The checkpoint must fire BEFORE any write attempt, not after the delete.** Correct post-verification sequence: verify → correct → sync → STOP. Do NOT reach for `cat > .json` or `json.dump()` after sync. See `references/session-20260624-light-gotcha73-violation.md`.
  - **Second duplicate variant (confirmed 2026-06-29):** After all verification and sync completed, the caller wrote a SECOND journal at `mentor-light-20260629T005540Z.json` (different run_id from script's `T005139Z`) because it rationalized the script's journal was "a different run." Caught and deleted post-hoc. Root cause: the checkpoint was advisory ("RESIST") rather than a structural gate. **Fix: Added `ls "$JOURNAL_DIR" | grep "mentor-light-" | wc -l` pre-check — if count ≥ 1, script already wrote, DO NOT WRITE.** See `references/session-20260629-light-journalization-violation.md`.
...
32a. **Deep heartbeat journal filename double-prefix bug** — `cron-heartbeat-deep-dualpath.py` composes the journal filename as `f"mentor-deep-{run_id}.json"` where `run_id = f"deep-{timestamp}"`. The result is `mentor-deep-deep-2026-06-23T194032Z.json` — a double-prefix. The `run_id` field inside the file is correct (`deep-2026-06-23T194032Z`), but the filename is redundant. This is a cosmetic issue in the script's journal writer. **Fix:** Change the filename composition to `f"{run_id}.json"` since `run_id` already contains the `deep-` prefix. Confirmed 2026-06-23.

## Correction Script — Use the Existing Script, Not Ad-Hoc

74. **Caller MUST use `correct_active_skills_30d.py` for the correction — never write ad-hoc correction scripts** — When the caller needs to correct `active_skills_30d` after a heartbeat, the existing script at `scripts/correct_active_skills_30d.py` is the authoritative tool. It reads the last evidence record, computes the true dual-path 30-day count, and appends a properly formatted correction record. Writing ad-hoc Python scripts to `/tmp/` for this purpose has produced: (a) duplicate evidence entries (4 instead of 2 in one heartbeat), (b) malformed run_ids (full ISO timestamp with microseconds instead of `YYYYMMDDTHHMMSSZ`), (c) subagent clobbering of `/tmp/` files producing further duplicates. **The correction is mandatory every time** — run `python3 {skill_dir}/scripts/correct_active_skills_30d.py` after every heartbeat. Do not write one-off correction scripts. Confirmed 2026-06-22: ad-hoc correction produced 4 evidence lines (2 script + 2 ad-hoc with duplicates), required manual repair to remove 2 malformed entries.

75. **Subagent `/tmp/` file clobbering in multi-skill dispatch** — When the dispatcher runs multiple pipelines concurrently (e.g., Forge + Mentor + Praxis), subagents from different pipelines can overwrite each other's `/tmp/` files. The `write_file` tool will warn: "was modified by sibling subagent." **Fix:** Use unique `/tmp/` filenames per pipeline (e.g., `/tmp/mentor_evidence_correction.py` instead of `/tmp/evidence_correction.py`), or use the `write_file` tool's overwrite protection by reading the file first before writing. For the standard correction workflow, use `correct_active_skills_30d.py` directly — it writes to the profile path, not `/tmp/`. Confirmed 2026-06-22: subagent from a sibling pipeline modified `/tmp/mentor_correct_evidence.py` while the mentor pipeline was writing it.

76. **Pre-run counts captured in a separate `terminal()` call are stale — concurrent writes corrupt the baseline** — When you capture `EVIDENCE_BEFORE=$(wc -l < file)` in one `terminal()` call and run the heartbeat script in a subsequent call, other processes (the prior heartbeat's correction script, concurrent cron jobs, or the dispatcher's other pipeline) can write to evidence.jsonl between your snapshot and the script's execution. The resulting delta is meaningless — it reflects accumulated writes from multiple processes/heartbeats, not just this run's output. In this session: separate-call pre-run count produced a delta of 4672 (meaningless) instead of the expected 2–3. **Fix:** ALWAYS capture pre-run counts, run the script, and verify post-run counts in a SINGLE `terminal()` call. Do NOT split the verify-and-backup workflow across multiple `terminal()` calls. This is distinct from gotcha #49 (shell variables not persisting across calls) — this is about file-level race conditions, not shell variable scope. Confirmed 2026-06-24.

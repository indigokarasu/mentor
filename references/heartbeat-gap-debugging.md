...
2. Check if mentor:light job ran: `terminal(command="cat <hermes-root>/cron/jobs.json | python3 -c \"import json,sys; d=json.load(sys.stdin); jobs=d.get('jobs',d.get('tasks',[])); [print(j.get('name'), j.get('last_status'), j.get('last_run_at')) for j in jobs if 'mentor' in j.get('name','')]\"")`
3. Check cron_jobs.log for the gap window: `terminal(command="grep mentor <hermes-root>/logs/cron_jobs.log | tail -20")`

...
**`cron-heartbeat-light.py` `JOURNALS_DIR` constant is legacy/unused for output (confirmed 2026-07-13):** The script hardcodes `JOURNALS_DIR = "<hermes-root>/commons/journals"` at module top (line 9), which looks like it would make the heartbeat read/write against the legacy root. It does NOT. The script reads its journal file list from **stdin** (the `find ... > /tmp/mentor_files_3d.txt && python3 cron-heartbeat-light.py < /tmp/mentor_files_3d.txt` pattern) and writes its own journal to `JOURNAL_DIR = os.path.join(AGENT_ROOT, "commons", "journals", "ocas-mentor")` where `AGENT_ROOT = "<hermes-home>"` — i.e. the **profile-scoped** tree. The legacy `<hermes-root>/commons/journals/ocas-mentor/` holds only 1 stale file (2026-07-12); the 138 active `mentor-light-*` journals all live under the profile path. Do NOT conclude the script leaks stray journals into the legacy tree, and do NOT "fix" the script by changing `JOURNALS_DIR` (it is only referenced in the docstring/usage example, not in execution). The dual-path `find` (both commons + profile) is what feeds stdin — see `references/dual-path-journal-discovery.md`.

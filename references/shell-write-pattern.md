...
printf '%s\n' '{"file": "/path/to/journal.json", "skill_name": "ocas-elephas", "ingested_at": "2026-06-05T05:47:00+00:00", "entries": 1}' >> <hermes-root>/commons/data/mentor/ingestion_log.jsonl

# ALSO OK: echo with explicit newline check
echo '{"file": "/path/to/journal.json"}' >> <hermes-root>/commons/data/mentor/ingestion_log.jsonl
# Then verify the last line is valid JSON:
tail -1 <hermes-root>/commons/data/mentor/ingestion_log.jsonl | python3 -c "import sys,json; json.loads(sys.stdin.read()); print('OK')"
```

### Write a full JSON file (journal, evidence)

```bash
cat > <hermes-home>/commons/journals/ocas-mentor/2026-06-05/20260605T054713Z.json << 'EOF'
{
...
cat /tmp/mentor_ingest_lines.txt >> <hermes-root>/commons/data/mentor/ingestion_log.jsonl
```

### Verify persistence

```bash
wc -l <hermes-root>/commons/data/mentor/ingestion_log.jsonl
wc -l <hermes-root>/commons/data/mentor/evidence.jsonl
```
...
**CRITICAL: Do NOT use `os.environ.get()` for metric values in backup evidence Python heredocs** — Shell variables set in the same `terminal()` block (e.g., `TOTAL_3D=959`) are NOT visible to `os.environ.get()` inside `python3 << 'PYEOF'` because single-quoted heredocs prevent shell expansion and non-exported vars don't propagate to subprocesses. The evidence record will have all-zero metrics. **Fix:** Compute values inside Python using subprocess:
    ...
    if os.path.getsize(tmp_path) > 0:
        with open('<hermes-root>/commons/data/mentor/evidence.jsonl', 'a') as f:
...
DATA_DIR = "<hermes-root>/commons/data/mentor"
record = {...}
...
with open('/tmp/mentor_ingest_backfill.txt', 'w') as out:
    for l in lines:
        out.write(l + '\n')
"
    cat /tmp/mentor_ingest_backfill.txt >> <hermes-root>/commons/data/mentor/ingestion_log.jsonl
fi

# === MANDATORY: Verify final state ===
tail -1 <hermes-root>/commons/data/mentor/evidence.jsonl | python3 -c "import sys,json; json.loads(sys.stdin.read()); print('Evidence OK')" || echo "EVIDENCE CORRUPT"
tail -1 <hermes-root>/commons/data/mentor/ingestion_log.jsonl | python3 -c "import sys,json; json.loads(sys.stdin.read()); print('Ingestion OK')" || echo "INGESTION CORRUPT"
...
The ingestion log stores paths in TWO formats: (a) relative `ocas-xxx/YYYY-MM-DD/file.json` and (b) absolute `<hermes-root>/commons/journals/ocas-xxx/...`. Profile-scoped paths use `<hermes-home>/commons/journals/ocas-xxx/...`. A naive `comm` between `find` output and ingestion log will show 800+ false "new" files. Fix:
...
with open('<hermes-root>/commons/data/mentor/ingestion_log.jsonl') as f:
    for line in f:
        ...
        try:
            ...
            elif fp.startswith('commons/'): ingested.add(os.path.join('<hermes-home>', fp))

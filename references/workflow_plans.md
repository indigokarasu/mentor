...
Plans are pre-authored, parameterized task sequences stored at `{agent_root}/commons/data/ocas-mentor/plans/*.plan.md`. Each plan defines an ordered set of steps, the skill and command each step invokes, what inputs each step receives, and what outputs it produces for downstream steps.
...
Read the plan file at `{agent_root}/commons/data/ocas-mentor/plans/{plan_id}.plan.md`.
...
Create directory `{agent_root}/commons/data/ocas-mentor/plan-runs/{plan_run_id}/`.
...
1. Read `state.json` from `{agent_root}/commons/data/ocas-mentor/plan-runs/{plan_run_id}/`.
...
Plans in `{agent_root}/commons/data/ocas-mentor/plans/` (discovered at runtime):
